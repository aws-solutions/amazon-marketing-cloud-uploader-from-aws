...
def test_is_timeseries(mock_env_variables):
    ...
    assert (
        _is_timeseries("amc/dataset_id/timeseries_partition_size/"+os.environ['AMC_ENDPOINT_URL']+"/filename")
        is False
    )
    assert (
        _is_timeseries("amc/dataset_id/PT1M/"+os.environ['AMC_ENDPOINT_URL']+"/filename")
        is True
    )
    assert (
        _is_timeseries("amc/dataset_id/PT1H/"+os.environ['AMC_ENDPOINT_URL']+"/filename")
        is True
    )
    assert (
        _is_timeseries("amc/dataset_id/P1D/"+os.environ['AMC_ENDPOINT_URL']+"/filename")
        is True
    )
    assert (
        _is_timeseries("amc/dataset_id/P7D/"+os.environ['AMC_ENDPOINT_URL']+"/filename")
        is True
    )
