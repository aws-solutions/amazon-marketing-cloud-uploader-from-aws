...
def fake_context():
    return MagicMock(
        log_stream_name="fake_log_stream"
    )
...
def test_send_response(mock_response, fake_event, fake_context, test_configs):
    ...
    send_response(event=fake_event, context=fake_context, response_status=test_configs["response_status"], response_data={})
...
def test_write_to_s3(fake_event, fake_context, test_configs):
    ...
    write_to_s3(event=fake_event, context=fake_context, bucket=test_configs["s3_bucket"], key=test_configs["s3_key"], body={})
...
@mock_s3
@patch("urllib.request.build_opener")
def test_copy_source_fail_env_key(mock_response, mock_env_variables, fake_event, fake_context, test_configs):
...
@patch("urllib.request.build_opener")
def test_copy_source_else(mock_response, mock_env_variables, fake_event, fake_context, test_configs):
    with mock_s3():
        s3 = boto3.client("s3")
        s3.create_bucket(Bucket=test_configs["s3_bucket"])
        s3 = boto3.resource("s3")
        s3_object = s3.Object(
            test_configs["s3_bucket"], f'{test_configs["s3_prefix"]}/{test_configs["s3_key"]}'
        )
        s3_object.put(Body="{}", ContentType=test_configs["content_type"])
        file_loc = "./webapp-manifest.json"
        manifest_data = [test_configs["s3_key"]]
        from helper.website_helper import copy_source
        with patch("builtins.open", mock_open(read_data=json.dumps(manifest_data))) as mock_file:
            copy_source(event=fake_event, context=fake_context)
